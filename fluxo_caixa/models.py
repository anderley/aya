...
class FluxoCaixa(models.Model):
    ...
    centro_custo = models.ForeignKey(
        CentroCusto, null=True, on_delete=models.DO_NOTHING,
        verbose_name="Centro de Custo"
    )
    valor = models.DecimalField(
        max_digits=19, decimal_places=2, verbose_name="Valor"
    )
    valor_moeda_estrangeira = models.DecimalField(
        max_digits=19, decimal_places=2, null=True,
        default=None, verbose_name="Moeda Estrangeira"
    )
    valor_cotacao = models.DecimalField(
        max_digits=19, decimal_places=2, null=True,
        default=None, verbose_name="Cotação Moeda"
    )
    data_vencimento = models.DateField(verbose_name="Data Vencimento")
    data_pagamento = models.DateField(
        null=True, verbose_name="Data Pagamento"
    )
    banco = models.ForeignKey(
        Banco, null=True, on_delete=models.DO_NOTHING, verbose_name="Banco"
    )
    projeto = models.CharField(
        max_length=50, null=True, verbose_name="Projeto"
    )
    forma_pagamento = models.CharField(
        max_length=30, choices=FormasPagamentos.choices,
        null=True, verbose_name="Forma Pagamento"
    )
    num_documento = models.CharField(
        max_length=50, null=True, default=None, verbose_name="Número Documento"
    )
    num_parcela = models.IntegerField(
        null=True, default=1, verbose_name="Número Parcela"
    )
    parcelas = models.IntegerField(
        null=True, default=1, verbose_name="Parcelas"
    )
    arquivo = models.FileField(
        upload_to="aya/fluxo-caixa/arquivos",
        null=True, blank=True, verbose_name="Arquivo")
    parcela_principal = models.ForeignKey(
        'FluxoCaixa', null=True, blank=True,
        on_delete=models.DO_NOTHING, verbose_name="Parcela Principal"
    )
